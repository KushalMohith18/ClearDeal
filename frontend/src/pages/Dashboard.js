...
const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Handle both response formats for company
      if (companyRes) {
        const companyData = companyRes.data?.company || companyRes.data;
        setCompany(companyData);
      }
      // Allow both owners and brand_managers to manage reps
      if (user?.company_id && (user?.role === 'owner' || user?.role === 'brand_manager')) {
        const repsRes = await api.get(`/companies/${user.company_id}/reps`).catch(() => ({ data: { reps: [] } }));
        // Handle both response formats for reps
        const repsData = repsRes.data?.reps || repsRes.data || [];
        ...
        {stats?.role === 'brand_manager' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Invite Reps for Brand Managers */}
              <div className="bg-white border border-slate-200 rounded-lg">
                <div className="px-5 py-4 border-b border-slate-100">
                  <h3 className="font-semibold text-slate-900" style={{ fontFamily: "'Public Sans', sans-serif" }}>Invite Negotiation Rep</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Reps negotiate deals on your behalf</p>
                </div>
                <div className="p-5">
                  <form onSubmit={handleInviteRep} className="space-y-3">
                    <input
            ...
            {/* Rep Management for Brand Managers */}
            {reps.length > 0 && (
              <div className="bg-white border border-slate-200 rounded-lg" data-testid="rep-management-section">
                <div className="px-5 py-4 border-b border-slate-100">
                  <h3 className="font-semibold text-slate-900" style={{ fontFamily: "'Public Sans', sans-serif" }}>Team Reps ({reps.length})</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Manage your negotiation reps and their budget limits</p>
                </div>
                <div className="divide-y divide-slate-50">
                  {reps.map(rep => (
                    <div key={rep.id} className="px-5 py-3" data-testid={`rep-row-${rep.id}`}>
